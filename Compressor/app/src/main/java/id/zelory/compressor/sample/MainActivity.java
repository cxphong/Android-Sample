...
public class MainActivity extends AppCompatActivity {
    ...
    public void customCompressImage(View view) {
        if (actualImage == null) {
            ...
            int maxWidth=800;
            int maxHeidth=1200;
            if(actualImage.length()/1024>600.0f) {
                 do{

                    // Compress image in main thread using custom Compressor
                    compressedImage = new Compressor.Builder(this)
                            .setMaxWidth(maxWidth)
                            .setMaxHeight(maxHeidth)
                            .setQuality(quality)
                            .build()
                            .compressToFile(actualImage);
                     quality-=2;

                }while (compressedImage.length() / 1024 > 600.0f);

                while (compressedImage.length() / 1024 < 500.0f){
                    compressedImage = new Compressor.Builder(this)
                            .setMaxWidth(maxWidth+=10)
                            .setMaxHeight(maxHeidth+=10)
                            .setQuality(quality)
                            .build()
                            .compressToFile(actualImage);
                }
